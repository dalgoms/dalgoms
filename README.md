...
## Core Results
* **14x** 리드 성장 및 **8x** ROAS 달성 (연간 리드 **3만 건** 확보)
* **연간 8~15억** 규모의 광고 예산 운용 및 최적화 경험
* **250+** 기업 고객(Timblo) 대상 서비스 및 솔루션 운영
* **10년+** 마케팅 전략 수립 및 시스템 자동화 경력

---

## What I Do
**"전략이 실제 결과로 이어지도록, 실행의 병목을 찾아 시스템을 설계합니다."**

단순히 광고를 세팅하거나 웹사이트를 만드는 일회성 작업에 그치지 않습니다. 유입부터 전환까지의 모든 여정이 하나의 유기적인 시스템으로 굴러가게 만드는 데 집중합니다.

**지속 가능한 성장을 위한 시스템 설계**
데이터와 가설 검증이 반복되는 구조를 만듭니다. 반짝하고 사라지는 캠페인이 아니라, 시간이 지날수록 데이터가 쌓이고 성과가 복리로 돌아오는 운영 체계를 구축하는 것을 지향합니다.

**병목 제거를 통한 흐름의 유연화**
대규모 예산을 집행하고 수만 건의 리드를 확보하는 과정에는 반드시 병목이 생기기 마련입니다. 저는 그 지점을 구조적으로 진단해 흐름을 뚫어주고, 예산과 리소스가 낭비 없이 성과로 흐르도록 최적화합니다.

**자동화를 통한 리소스의 선순환**
단순 반복 업무는 시스템과 자동화에 맡깁니다. 여기서 확보한 여유 시간은 더 날카로운 신규 기획과 탄탄한 운영 구조를 만드는 데 다시 투자합니다. 이 선순환이 팀의 실행 속도와 생산성을 압도적으로 높인다고 믿습니다.

**데이터로 증명하는 최적화**
감에 의존하지 않고 A/B 테스트와 유입 경로 분석을 통해 랜딩 페이지를 끊임없이 재설계합니다. 유입된 고객이 이탈 없이 전환으로 이어지도록 집요하게 구조를 다듬어 비즈니스 임팩트를 만들어냅니다.

같은 방향의 문제의식을 가진 팀과 함께할 때 가장 큰 시너지를 냅니다. 시스템의 힘으로 전략을 현실화하는 여정에 진심입니다.
💡 달라진 점
---
...
|:---:|:---:|:---:|
| 데이터 → IA · 메시지 · 전환 구조 설계 | 도메인 → CMS → DB → 리드 플로우 | 병목 → 자동화 → 실행 가속 |
