...
반복되는 병목이 보이면 도구를 만듭니다. 만든 도구는 팀에 배포하고, 실제 업무에 쓰고 있습니다.
...
URL을 넣으면 사이트 구조를 자동 크롤링하고, GPT-4o가 진단 리포트를 생성하는 도구.

**문제** — 경쟁사 사이트를 하나하나 수동으로 분석하는 데 시간이 과도하게 소요됨.

**해결** — 사이트 구조 크롤링 + GPT-4o 진단 리포트 자동 생성을 하나의 도구로 만들었습니다. 팀원이 URL만 입력하면 리포트를 받을 수 있습니다.
...
AI 카피 생성 → 템플릿 렌더링 → 멀티사이즈 자동 내보내기.

**문제** — 플랫폼별로 소재를 일일이 만드는 반복 작업.

**해결** — AI가 카피를 쓰고, 템플릿에 렌더링하고, 여러 사이즈로 한번에 내보내는 파이프라인을 구축했습니다.
...
**해결** — 한글 접두어 파싱, 워크플로우 템플릿, AI 업무 분해, 완료 체인으로 텔레그램 한 줄 입력만으로 노션 DB에 구조화된 할일이 생성됩니다.
...
**해결** — AI 스마트 분류, 중복 파일 탐지, 버전 체인 분석, 용량 분석, 연도별 아카이브 제안으로 텔레그램 한 줄이면 폴더를 진단하고 정리합니다. 집/회사 PC 멀티 기기 지원.
...
- **Analysis** — UTM · A/B Testing · 퍼널 분석
...
Next.js · TypeScript · GPT · Make.com · Notion API · Wix Velo · Vercel · GA4
